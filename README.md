...
- 3 行（原表 BM4-Analysis の ψ(I) までの行）: 行列から直接ではなく、原表の UNOCF 列の順序数表記を
  ...
  添字の ψ_1(Ω_{Ω_2}+Y) ↦ ψ_2(Y')、2 つ目以降の Ω_{Ω_2} の段の項 ↦ Ω_{ψ_2(…)}、一段上（Ω_{Ω_3}）も同様。
  これで原表の 2574〜2795 行の像が変わる（例 ψ_0(Ω_{Ω_2}·2) ↦ ι(ψ_0(Ω_{ψ_2(Ω_{ψ_1(Ω_{Ω_2}·2)})}))）。
- 4 行以上: 未対応。
...
- 未解決: 180 行は検査を通るが近似列に非標準形を含み、上限の照合が弱い（ι だけでは 214 行）。
  低い崩壊の引数の中に Ω_{Ω_2} を含む型（例 ψ_0(Ω_{Ω_2}+ψ_2(Ω_{Ω_2}))）、ψ_0(Ω_{Ω_3}+Ω_{Ω_2})、Ω_{Ω_{ω+1}} 以上の段など。
