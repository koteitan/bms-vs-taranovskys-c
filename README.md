...
- 3 行: 原表 4503 行のうち 4492 行が `tools/ebp_lab.js` の検査（翻訳 f = ι∘σ）で指摘なし。
...
- 未解決: 132 行は検査を通るが近似列に非標準形を含み、上限の照合が弱い（ι だけでは 214 行）。
  低い崩壊の引数の中に Ω_{Ω_k} を含む型（例 ψ_0(Ω_{Ω_2}+ψ_2(Ω_{Ω_2}))、ψ_0(Ω_{Ω_ω}+ψ_ω(Ω_{Ω_2}))）、ψ_0(Ω_{Ω_3}+Ω_{Ω_2}) など。
