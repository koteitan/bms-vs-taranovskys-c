...
- 2 行: 規則 R2n・R2g・R1d で、`tools/rule_lab.js` の極限 313 個（原表と展開サンプル）のうち 309 個が検査
  （上限一致・近似列より上・標準形）を通過し、近似列が標準な失敗は 0 個。
- 3 行: 原表の表記経由で 4484 行（ψ(I) の行を含む）。4478 行は `tools/ebp_lab.js` の検査で指摘なし。
  指摘ありは ψ_ω の中で最後の項が Ω_{ω+k} の型、添字に ψ を含む型、原表の表記の順序が BMS の並びと逆の行（48 行）など。
