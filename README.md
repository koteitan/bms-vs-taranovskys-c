...
- 2 行（ペア数列）: P進大好きbot 氏の写像で Buchholz の表記 (0, D_ν, +) に写し、
  ...
  規則 N2 で指数を ψ̂_{ν-1}(Ω_ν + a) に置き換える。ψ_0(α' + ψ_μ(b))（μ ≥ 2）の最後の項は規則 R2u で扱う
  （b が後続なら和のまま、b = α' + (段 μ の項) なら文脈つきの ι、それ以外は ψ̂_{μ-1}(a) に潰す）。
...
- 2 行: 規則 R2n で、`tools/rule_lab.js` の極限 313 個（原表と展開サンプル）のうち 303 個が検査（上限一致・近似列より上・標準形）を通過。
  残りは ψ_0(Ω_3 + ψ_2(Ω_3 + ψ_2(Ω_3 + ψ_2(Ω_2)))) のような入れ子の型。
- 3 行: 原表の表記経由で 4484 行（ψ(I) の行を含む）。4452 行は `tools/ebp_lab.js` の検査で指摘なし。
  指摘ありは ψ_0(α' + ψ_μ(α')) の型、ψ_1 の中の同種の型、原表の表記の順序が BMS の並びと逆の行（48 行）など。
