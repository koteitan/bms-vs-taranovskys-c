...
- 3 行（原表 BM4-Analysis の ψ(I) までの行）: 行列から直接ではなく、原表の UNOCF 列の順序数表記を
  ...
  ι だけでは、添字の中に ψ_1(Ω_{Ω_2}) のような崩壊を含む項を C の標準形に写せず（規則 R4）、その先の像の値がずれる。
  そこで翻訳は f = ι∘σ とし、σ（`ebp_sigma.js`）で項を順序を保って ι が扱える項へ送ってから写す:
  添字の ψ_1(Ω_{Ω_2}+Y) ↦ ψ_2(Y')、2 つ目以降の Ω_{Ω_2} の段の項 ↦ Ω_{ψ_2(…)}。段 κ は引数の先頭の項の添字 Ω_{κ+1} から読み、
  有限の段（Ω_{Ω_3} など）にも超限の段（Ω_{Ω_{ω+1}}、Ω_{Ω_{Ω+1}} など）にも同じ規則を当てる。
  先頭がより大きい項のときは、それを 1 つ目の Ω_{Ω_{κ+1}} とみなす（例 ψ_0(Ω_{Ω_3}+Ω_{Ω_2}) ↦ ψ_0(Ω_{Ω_3}+Ω_{ψ_2(Ω_{ψ_1(Ω_{Ω_2}·2)})})）
  （例 ψ_0(Ω_{Ω_2}·2) ↦ ι(ψ_0(Ω_{ψ_2(Ω_{ψ_1(Ω_{Ω_2}·2)})}))）。
- 4 行以上: 未対応。
...
- `tools/ebp_std_rule.js`: 拡張 Buchholz ψ の項 t について「ι(t) が C の標準形か」を規則 R4 で判定し、実際の判定と照合する
  （3 行の原表 2000〜4506 行と近似列の正規形 10110 件で完全一致）。R4 は、添字の中の崩壊 ψ_κ(X)（κ ≥ 1）が、
  外側により低い崩壊を持ち、X が一番外側の添字の主項 s について Ω_{s+1} 以上のときに除外する。
...
- 3 行: 原表 4503 行のうち 4493 行が `tools/ebp_lab.js` の検査（翻訳 f = ι∘σ）で指摘なし。
  指摘ありは原表の表記の順序が BMS の並びと逆の行（48 行）、表記が標準形でない行など。
- σ の効果: 行 2574 ψ_0(Ω_{Ω_2}+Ω_{ψ_1(Ω_{Ω_2})}) の近似列の上限が ι だけの ψ_0(Ω_{Ω_2}·2) の像と一致し、
  その先の像がずれていた。σ を入れると 2574〜2795 行の 210 行の像が変わり、悪くなる行はない（`tools/sigma_lab.js`）。
- 未解決: 108 行は検査を通るが近似列に非標準形を含み、上限の照合が弱い（ι だけでは 214 行）。
  低い崩壊の引数の中に Ω_{Ω_k} を含む型（例 ψ_0(Ω_{Ω_2}+ψ_2(Ω_{Ω_2}))、ψ_0(Ω_{Ω_ω}+ψ_ω(Ω_{Ω_2}))）など。
