...
//
// 段 k（k = 1, 2）ごとに、ψ_0 の引数 a の先頭が Ω_{Ω_{k+1}} の段の項のとき:
//   - 添字の主項 ψ_k(大きい項 + Y) ↦ ψ_{k+1}(λ)
//   - 2 つ目以降の段 k の項 q ↦ Ω_{ψ_{k+1}(λ)}
//   λ(b): b の先頭の Ω_{Ω_{k+1}} を落とし、残りの大きい項 y_m を Ω_{ψ_k(b_{≤m})} に置き換えた和
//         （k ≥ 2 では y の浅い位置の Ω_k を ψ_{k-1}(y) に置き換えてから）
...
//
// 原表の 3 行（2000〜4506 行）で、今の ι と比べて悪くなる行はなく、2574〜2795 行の 210 行の像が変わる（tools/sigma_lab.js）。
'use strict';
...
// 段 k の定数: W = Ω_{k+1}, OO = Ω_{Ω_{k+1}}, ETA = ψ_k(Ω_{Ω_{k+1}}),
...
// opts: levels（既定 [1, 2]）, low（低い崩壊の引数にも当てる、既定 false）,
//       eorig（最後の低い項の E を σ の前の引数から作る、既定 true）, r2nu（既定 true）
function makeSigma(opts = {}) {
  const LEVELS = (opts.levels || [1, 2]).map(level);
  const LOW = !!opts.low;
  ...
  //   y の添字の主項としての Ω_k、その主項の引数の最上位の項としての Ω_k、a の最上位の項 ψ_{Ω_k}(..) の添字 Ω_k
  function normE(L, y) {
    const Wk = P(nat(L.k), 0), Ek = P(nat(L.k - 1), y);
  ...
  function lam(L, b) {
    if (L.k >= 2) b = sum(terms(b).map(y => (isBig(L, y) && !eq(y, L.OO)) ? normE(L, y) : y));
