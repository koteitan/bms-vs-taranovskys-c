...
- 素直な木の読み（列 v ↦ D_ν(v)(1 行目の子の和)）を今の ι にかけると、列 6 個までの 2 行 1346 個中 1144 個が tr と一致。
  外れは対角 (1,1)(2,2)(3,3) が 1 項 D_3 0 にまとまる所。まとまる列を単に飛ばすと、(1,1)(2,2)(2,0) のように
  transPS が「上に出した項 + 写し」に組み替える所で外れる（1050 個）。Buchholz の形で組むと正規形の組み替えを真似することになるので、C の鎖で直接組む。
- 2 行の木の規則 v0（`direct.js` の `tree2`）: 列 v の段 ν(v) = 0（2 行目が 0）、ν(2 行目の親) + 1。
  ν = 0 の節の値は、2 行目が正の子ごとに P = C(e, P)（e は段 1 の鎖、土台 Ω_1）、2 行目が 0 の子の鎖 L があれば C(L, P)、
  2 行目が正の子がなければ C(L, 0)。鎖の中の ν = 0 の列の指数は log(節の値)。段 L の節の子は、対角で合流する子
  （u のすぐ次の列で 2 行目の親が u、かつ u の 2 行目の親 = u の 1 行目の親）なら土台を C(Ω_2, 土台) にして同じ鎖を続け、
  それ以外は x（2 行目が正なら自分の段の Ω̂ を土台にした鎖、0 なら log(節の値)）を積む（段 1 は C(x, 土台)、段 2 以上は内側の鎖 I で C(I, 土台)）。
  結果: `direct_enum.js 6` で 1265 個中 1039 個が tr と一致、`direct_lab.js --rows 2` で 575 個中 167 個が全部の検査を通る。
  残り: 段 2 以上の項を鎖の要素として使うときは log の形（ι の expT）、値として使うときは C(I, Ω̂) の形に分ける必要がある
  （例 (0,0)(1,1)(2,2)(3,3)(2,2)(3,0)）。N2（段 ν ≥ 2 の節の最後の同じ段の項の指数を ψ̂_{ν-1}(Ω_ν + a) に置き換える、例 (0,0)(1,1)(2,2)(3,2)）が未実装。
- `direct_lab.js` の 1 行: 行列 98 個（極限 97 個）すべてが検査を通る。
...
| ファイル | 区分 | 役割 |
|---|---|---|
| `../direct.js` | 直接 | 行列から直接の翻訳（作業中）。和の分解、1 行の木の規則、2 行の木の規則 v0。3 行以上の木はまだ null |
| `direct_enum.js` | 直接 | `direct.js` を、列 N 個までの標準な 2 行の行列の総当たりで `tr` と照らし合わせる |
